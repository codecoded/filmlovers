module UserScopes
  ...
  def recommendations_view_for(user_id, state=:recommended)
    joins(:film_recommendations).group('users.id').where('film_recommendations.friend_id = ? and film_recommendations.state = ?', 6, :recommended).select('users.*, count(film_recommendations.id)')
  end
